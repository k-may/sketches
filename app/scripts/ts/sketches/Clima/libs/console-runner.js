...
(function(jasmine, console) {
  ...
  proto.reportRunnerStarting = function(runner) {
    ...
    this.passed_specs = 0;
    this.log("ConsoleReporter starting...");
  };
  ...
  proto.reportSpecResults = function(spec) {
    ...
    for (var i = 0; i < items.length; i++) {
      var trace = items[i].trace.stack || items[i].message || items[i].trace;
      this.log(trace, "red");
    }
  };
  ...
  proto.log = function(str, color) {
    var text = (this.inColour && color != undefined)? ANSI.colorize_text(str, color) : str;
    console.log(text)
