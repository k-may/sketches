    ...
    function (Utils,
        ...
        return BaseSketch.extend({
            ...
            draw:function () {
                ...
                var speed = (Math.sin(time*0.001));
//console.log(speed)
                this.planes[0].pos.x = pos.x;
                this.planes[0].pos.y = pos.y;
                for (var i = 0; i < this.planes.length; i++) {
                    var plane = this.planes[i];
                    plane.pos.x += (pos.x - plane.pos.x) * 0.2;// * (speed + 1)/2;
                    plane.pos.y += (pos.y - plane.pos.y) * 0.2;// * (speed + 1)/2;
                    plane.pos.z = (plane.pos.z + 5 * speed) % MAX_DEPTH;
                    pos = plane.pos;
                }

                speed = (Math.sin(time*0.0005));// * -1;
                var index = Math.floor((speed + 1)/2* (this.planes.length - 1));

                // if (Math.floor(time / 2000) % 2) {
                if(speed < 0){
                    this.drawForward(pos,index);
                } else {
                    this.drawReversed(pos, index);//this.planes.length - 1);
                }
            },

            drawForward:function (pos, index) {
                var z = 0;
                ...
                for (var i = index; i < this.planes.length; i++) {
                    var plane = this.planes[i];
            ...
            drawReversed:function (pos, index) {
                var z = 0;
            ...
            scalePt:function (pt,scale) {
                pt.x = Math.abs(pt.x * scale);
                pt.y = Math.abs(pt.y * scale);
            },
            ...
            rotatePt: function(pt, rads){
                pt.x = Math.sin(rads)*pt.x;
                pt.y = Math.cos(rads)*pt.y;
